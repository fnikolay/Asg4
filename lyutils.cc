...
void scanner_newline (void) {
   ++scan_linenr;
   //fprintf(test, "The line number is: %d\n\n", scan_linenr);
   scan_offset = 0;
}

int get_file_num(){
    return included_filenames.size() - 1;
    //return file_num;
}
...
int yylval_token (int symbol) {
   ...
   fprintf(tok, "   %d  %2d.%-2.3d  %-4d  %-13s  (%s)\n",
      get_file_num (), get_linenum(), get_scan_offset(),
...
RCSC("$Id: lyutils.cc,v 1.2 2014-11-15 23:14:58-08 - - $")
